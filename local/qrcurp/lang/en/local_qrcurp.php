...
$string['validateexternalconnection'] = "Validar conexión a BD externa";
$string['validateexternalconnectioninfo'] = "Ejecuta una consulta de validación al crear la conexión para verificar que la BD externa responde correctamente.";
$string['externalconnectiontestquery'] = "Consulta de validación de conexión externa";
$string['externalconnectiontestqueryinfo'] = "Consulta SQL simple para validar la conexión (ejemplo: SELECT 1).";
...
$string['formextrafieldsinfo'] = "Un campo por línea con formato shortname|Etiqueta|tipo(text/email/number/date/checkbox/select)|requerido(1/0). Para select agrega 5o parámetro: opciones como valor:Texto,valor2:Texto2. Deben existir en profilefieldslist para guardarse en perfil.";
...
$string['hidesitechrome'] = "Ocultar header/footer del sitio";
$string['hidesitechromeinfo'] = "Si está activo, el formulario de registro se mostrará sin el header y footer del tema.";
$string['allowautofilledpasswordedit'] = "Permitir editar contraseña autocompletada";
$string['allowautofilledpasswordeditinfo'] = "Si está activo, cuando la contraseña se autocompleta desde datos externos el usuario podrá modificarla.";
$string['editableautofilledfields'] = "Campos autocompletados editables";
$string['editableautofilledfieldsinfo'] = "Lista separada por comas de IDs de campos que podrán editarse aunque se autocompleten (ejemplo: ocupacion,categorias,grupos,email).";
...
$string['publicogeneralblockedtext'] = 'Mensaje cuando no hay registro para público general';
$string['publicogeneralblockedtextinfo'] = 'Texto que se mostrará cuando el registro no esté disponible para público en general.';
...
$string['selectoption'] = 'Seleccionar';
$string['nogroupschedule'] = 'Sin Horarios';
$string['enrolmorewarning'] = 'Te recordamos que si has estado en una lengua en más de 2 trimestres, no tendras acceso a esa lenga y deberas escoger otra lengua que quieras tomar.';
$string['enrolmoreinvalidcurp'] = 'Verifica que tu CURP es correcta, o que te encuentras registrado en la plataforma, si estas seguro de que es correcta, contacta a el administrador del sitio: {$a}';
$string['enrolmoreonecourseonly'] = 'Solo puedes pertenecer a un curso del periodo actual';
$string['enrolmoretitle'] = 'Hola, actualmente te encuentras registrado en CVL, para poder registrarte en el nuevo perido o seleccionar una nueva lengua, por favor, ingresa tu CURP y posteriormente selecciona el curso y el grupo al que deseas pertenecer:';
$string['enrolcurp'] = 'Clave Única de Registro de Población (CURP):';
$string['enrolcourse'] = 'Curso:';
$string['enrolavailablegroups'] = 'Grupos disponibles:';
$string['enrolsubmit'] = 'Inscribirme';
$string['registrationclosed'] = 'La fecha de registro ha concluido, te sugerimos consultar la información para el próximo periodo.';
$string['mailwelcome'] = 'Bienvenido.';
$string['mailsubjectwelcomeplatform'] = 'Bienvenida(o) a {$a}';
$string['mailbodywelcomeplatform'] = 'Estimada/o <strong>{$a->firstname} {$a->lastname}</strong>.<br><br>Te damos la más cordial bienvenida al {$a->category}.<br><br>Tus datos de acceso son los siguientes:<br><br><strong>URL:</strong> <a href="{$a->url}" target="_blank">{$a->url}</a> <br> <strong>Nombre de usuario:</strong> {$a->username} <br> <strong>Contraseña:</strong> {$a->password}';
$string['mailsubjectwelcomecourseplatform'] = 'Bienvenida(o) a {$a->course} de {$a->platform}';
$string['mailbodywelcomecourse'] = 'Estimada/o <strong>{$a->firstname} {$a->lastname}</strong>.<br><br>Te damos la más cordial bienvenida a la comunidad de práctica de {$a->course}.<br><br>Tus datos de acceso son los siguientes:<br><br><strong>URL:</strong> <a href="{$a->url}" target="_blank">{$a->url}</a> <br> <strong>Nombre de usuario:</strong> {$a->username} <br> <strong>Contraseña:</strong> {$a->password}';
$string['mailsubjectwelcomecourse'] = 'Bienvenida a {$a}';
$string['mailbodywelcomecoursegroup'] = 'Estimada/o <strong>{$a->firstname} {$a->lastname}</strong>.<br><br>Te damos la más cordial bienvenida al curso de {$a->course}, en el grupo de {$a->group}.<br><br>Tus datos de acceso son los siguientes:<br><br><strong>URL:</strong> <a href="{$a->url}" target="_blank">{$a->url}</a> <br> <strong>Nombre de usuario:</strong> {$a->username} <br> <strong>Contraseña:</strong> {$a->password}<br><br>Agradecemos tu interés y participación.';
$string['mailsubjectconfirmregistration'] = '{$a} :: Confirmación de registro';
$string['mailbodyconfirmregistration'] = 'Hola, {$a->firstname} {$a->lastname}.<br><br>Se ha solicitado una nueva cuenta en \'{$a->category}\' utilizando su dirección de correo electrónico.<br><br>Para confirmar su nueva cuenta, copie y pegue la siguiente URL en la barra de direcciones (parte superior de la ventana) de su navegador web:<br><br><strong>{$a->url}</strong><br><br>Agradecemos tu interés y participación.';
$string['mailsubjectrecovercredentials'] = 'Recuperación de credenciales de acceso - {$a}';
$string['mailbodyrecovercredentials'] = 'Estimado {$a->name}:<br><br>Hemos recibido una petición de recuperación de Nombre de usuario y/o Contraseña en el sitio {$a->platform}<br><br>Estas son tus credenciales de acceso:<br><br><strong>URL:</strong> <a href="{$a->url}">{$a->url}</a> <br> <strong>Nombre de usuario:</strong> {$a->username} <br> <strong>Contraseña:</strong> {$a->password}<br><br>Para iniciar tu sesión, accede a la URL con las credenciales de acceso que te estamos enviando.<br><br><hr><br>';
$string['mailsubjectwelcomecourseplatformshort'] = 'Bienvenida a el curso de {$a->course}, {$a->platform}';
$string['mailbodywelcomereenrolcourse'] = 'Estimada/o <strong>{$a->firstname} {$a->lastname}</strong>.<br><br>Te damos la más cordial bienvenida a {$a->course}, en el grupo de {$a->group}.<br><br>Tus datos de acceso son los siguientes:<br><br><strong>URL:</strong> <a href="{$a->url}" target="_blank">{$a->url}</a> <br> <strong>Nombre de usuario:</strong> {$a->username} <br> <strong>Contraseña:</strong> {$a->password}<br><br>¡Para iniciar sesión deberás ingresar a la URL y autenticarte con tus datos de acceso!<br><br><hr>{$a->platform}<br>';
...
$string['enrolmoreperiodcategoryid'] = "Categoría para validar inscripción previa (enrolmore)";
$string['enrolmoreperiodcategoryidinfo'] = "Debe ser la categoría del periodo en el que se quiere validar si ya existe inscrito el usuario.";
...
$string['pausedregistrationprefix'] = 'Estimado participante, el período de registro se encuentra en pausa';
$string['pausedregistrationsuffix'] = 'revisa la página principal para obtener más información acerca del próximo período.';
