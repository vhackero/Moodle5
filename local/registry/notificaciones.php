...
function notifyregistry($id,$destination = "",$descripcioItem = ""){
    global $CFG;
    if($destination == ''){
        $destination = "$CFG->wwwroot/local/registry/index.php";
    }
    if($id == 1){
    ...
    if($id == 2){
        ...
        $message = "Los ids de cursos en específico se encuentra vacío, configurar correctamente el pluggin REGISTRY";
        redirect($destination, $message, 15, \core\output\notification::NOTIFY_WARNING);
    }
    if($id == 3){
        //Notificación para cuando esta activo el registro por patron pero no se selecciono un id
        $message = "Debes agregar un id en el patron a seguir para los grupos, configurar correctamente el pluggin REGISTRY";
        redirect($destination, $message, 15, \core\output\notification::NOTIFY_WARNING);
    }
