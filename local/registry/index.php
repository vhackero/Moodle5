...
global $DB,$PAGE,$CFG,$NAMEPLATAFORMREGISTRY,$NAMEEXTERNALDBREGISTRY;

$PAGE->set_url(new moodle_url('/local/registry/index.php'));
$PAGE->set_context(\context_system::instance());
$PAGE->set_title($NAMEPLATAFORMREGISTRY);
...
echo "<script>
        localStorage.setItem('nameExternalData','$NAMEEXTERNALDBREGISTRY');
        localStorage.setItem('namePlataform','$NAMEPLATAFORMREGISTRY');
...
$fechaLimiteRegistro = strtotime(get_config('local_registry','dateregistro'));
$fechaporperidos = get_config('local_registry','dateperiodos');
$fechaActual = strtotime(date('d-m-Y'));
$menssage = get_config('local_registry','textregistro');
...
$defaultcategory = get_config('local_registry','defaultcategoryid');
$defaultcourse = get_config('local_registry','defaultcourse');
$defaultgroup = get_config('local_registry','defaultgroup');
...
if($categoryid != '') {
    $rolStudent = get_config('local_registry','rolstudent');
    ...
    $totalAlumnoPorCategoria = get_config('local_registry','studentxcategory');

    if($numStundentToCategory == $totalAlumnoPorCategoria){
        $url = $CFG->wwwroot.'/index.php';
        $menssage = get_config('local_registry','studentxcategorytext');
        redirect($url, $menssage , 15, \core\output\notification::NOTIFY_WARNING);
    }
    ...
    $nameCategoria = get_config('local_registry','defaultnamecategory');
    $name = "iconos/".$nameCategoria.".jpg";
    ...
    if(get_config('local_registry','sampleregister') == 0) {
            ...
            <div style="display: none" id="dos_form" class="row">
                <div class="col-md-6 offset-md-3 card" id="medio" >
                    <div class="panel-heading">
                        <h1 id="texto-a-mostrar" >Por favor, teclea tu CURP. Si no la tienes consúltala aquí: <a target="_blank" href="https://www.gob.mx/curp/"><u>Consultar CURP.</u></a></h1>
                    </div>
                    <hr>
                    <form id="controler-curp" action="decode.php" method="post" enctype="multipart/form-data">
                        ...
                        <div id="muestra-curp" style=" padding: 0px 5% 10px;">
                            ...
                            <span>Si tienes dudas puedes consultar el manual dando </span><a target="_blank" href="docs/guia-registry.pdf"><b>clic aquí</b></a>
                        </div>
                    ...
                    <div id="controler-text-curp">
                        <form id="envia-info" action="decode.php" method="post" enctype="multipart/form-data" >
                            ...
                            <input id="continuar" type="submit" disabled class="btn btn-md btn-block btn-info" style="background-color: #611232 !important; color: white" value="Continuar">
