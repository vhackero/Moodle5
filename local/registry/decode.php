...
$PAGE->set_url(new moodle_url('/local/registry/decode.php'));
...
$contadorVisitas = get_config("local_registry","counterviews");
if($contadorVisitas){
    include_once "contadorvisitaspre-registro.php"; //comentar mientras se estén haciendo pruebas
}

//VARAIBLES GLOBALES USADAS
global $CFG,$DB,$DBEXTERNAL,$NAMEEXTERNALDBREGISTRY,$NAMEPLATAFORMREGISTRY;
echo $OUTPUT->header();
...
$gruoactive = get_config("local_registry",'creategroupenrol');
$gruoidcreate = get_config("local_registry",'idcreategroup');
$validaconrenapo = get_config("local_registry",'validaterenapo');
$omitevalidacionrenapo = get_config("local_registry",'acceptnotvalidaterenapo');

if($gruoactive == 1){
    if($typegrouping == ''){
        ($gruoidcreate == "")? notifyregistry(3,$CFG->wwwroot.'/local/registry/') : $typegrouping=$gruoidcreate;
    }
}
$categoryid = optional_param('categoryid','', PARAM_INT);   // Category id (defaults to Site).
$registropublicogeneral = get_config('local_registry','publicogeneral');   // Aceptara registros de publico externo.
$soloregistropublicogeneral = get_config('local_registry','onlypublicogeneral');   //Solo aceptara registros de publico externo
...
if($_POST['categoryid'] > 0){
    if($NAMEPLATAFORMREGISTRY == ''){
        //coloca el nombre de la categoría al nombre de la plataforma
        $NAMEPLATAFORMREGISTRY = $nameCategoria;
        echo "<script>localStorage.setItem('namePlataform','$NAMEPLATAFORMREGISTRY');</script>";
    }
}
if($categoryid == ''){$nameCategoria = get_config('local_registry','defaultnamecategory'); }
...
if($existeerror >0){
    ...
    redirect('index.php', get_string('dbconnerr','local_registry') , null, \core\output\notification::NOTIFY_INFO);
}else{
    $esinactivo = '';
    ...
    while($row = mysqli_fetch_array($datos)) {
        ...
        if($curp != ''){
            ...
            $muestramensaje = "Tus datos han sido registrados previamente en la base de datos de $NAMEEXTERNALDBREGISTRY, da clic en aceptar, revisa tu información antes de continuar con el registro $nameCategoria"; //MENSAJE A MOSTRAR
...
//        echo "El estado es :".$status;
    ...
    if($curp == ''){
        $despachador = 4;
        $muestramensaje = get_string('noregisexdb','local_registry');
    ...
    if ($curp == '' AND $estaregis != '' AND $remoteinsertdb == 0){
        ...
        $despachador = 3;
        $muestramensaje = get_string('regismoodlenotexdb0','local_registry');
    }
    if ($curp == '' AND $estaregis != '' AND $remoteinsertdb == 1){
        ...
        $muestramensaje = get_string('regismoodlenotexdb1','local_registry');
    }
    if ($curp != '' AND $estaregis != '' ){
        //esta registrado en moodle, y en bd externa
        $despachador = 3;
        //$soloundm = 1;//para cuando ya esta registrado en moodle pero no en undm y se requiere registar
        $muestramensaje = get_string('regismoodleyexdb','local_registry');
    ...
    if ($curp == '' AND $estaregis == '' AND $remoteinsertdb == 1){
        ...
        $muestramensaje = get_string('regismoodlenotexdb','local_registry');
    }
...
$correoenuso = get_string('correoregimoodle','local_registry');
...
?>
        ...
        `;
            ...
            $(document).ready(function () {
                ...
                if( yaestaregsitrado == true){
                    ...
                    elurl.innerHTML = "<a href='/local/registry/recovery/' target='_blank'>Clic aquí</a>"
                    ...
                    <form id="envia-info" action="<?= $registramoodle?>" method="post" enctype="multipart/form-data">
                        ...
                        <div id="opcionalesdocen" style="display: none">
                            ...
                            <label for="register-goals" class="focus-out">
                                <span class="label-text">Cuéntanos por qué te interesa <?=$NAMEPLATAFORMREGISTRY?></span>
                        ...
                        <label for="register-terms_of_service">
                            <input id="register-terms_of_service" type="checkbox" disabled name="terms_of_service" class="input-block checkbox check-size" required title="Debe aceptar los Términos de Servicio de <?=$NAMEPLATAFORMREGISTRY?>" >
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Selecciona el nav con las clases específicas
            var navbar = document.querySelector('nav.navbar.navbar-toggleable-md');
            if (navbar) {
                navbar.remove(); // Elimina el elemento del DOM
            }
        });
    </script>
<?php
