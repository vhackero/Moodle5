 ...
 */
class core_grades_group_external extends external_api {
    /**
     ...
     * Esta funcion fue customisada para recibir como parametro un id de grupo y no la lista de usuarios.
     *
     ...
     * @since Moodle 2.7
     */
    public static function get_grades($courseid, $component = null, $activityid = null, $groupid) {
        global $CFG, $USER, $DB;
        $params = self::validate_parameters(self::get_grades_parameters(),
            array('courseid' => $courseid, 'component' => $component, 'activityid' => $activityid, 'groupid' => $groupid));
        ...
        // ----------------- CUSTOM CODE MACUCO --------------------------
        $existeGrupo = groups_group_exists($params['groupid']);
		if(!$existeGrupo){
            throw new invalid_parameter_exception('Missing required key in single structure: groupid');
		}

//        $groupusers = groups_get_members($params['groupid'], 'u.id', 'u.id ASC');  //Obtiene los usuarios del grupo comentado este optien por el id
        $groupusers = groups_get_members($params['groupid'],'u.id','u.lastname ASC');  //Obtiene los usuarios del grupo ordenados por apellidos

		$usuarios = array();
        //LFAS 28-03-2024 Para validar que únicamente se envien los estudiantes
        $context = context_course::instance($params['courseid']);
        //FIN
		foreach ($groupusers as $idUsuario => $usuario){
            //LFAS 28-03-2024 Para validar que únicamente se envien los estudiantes
            $user_roles = get_user_roles($context, $idUsuario);
            $user_info = get_complete_user_data('id', $idUsuario);
            $patron = "/^es\d/i"; //para tomar en cuenta solo los nombres de usuario que comienzan con es
            if(!strstr($user_info->username,'estudiante')){
                if (preg_match($patron, $user_info->username)) {
                    foreach ($user_roles as $searchrole) {
                        if (strstr($searchrole->name, 'Estudiante UnADM')) {
                            if (!strstr($searchrole->name, 'Estudiante UnADM Pruebas')) {
                                array_push($usuarios, $idUsuario);
                                break;
                            }
                        }
                    }
		...
		//TODO Verificar que tenga usuarios el grupo, solo si es necesario

		$params['userids']=$usuarios;//Se asignan los usuarios a los parametros para que funcione naturalmente la funcion
		
		//---------------- FIN CUSTOM CODE MACUCO --------------------------
        ...
        if ($activitygrades = grade_item::fetch_all($gradeparams)) {
            ...
            foreach ($activitygrades as $activitygrade) {
                if(isset($CFG->ocultaencuestasaulacalificador) AND $CFG->ocultaencuestasaulacalificador == 1 AND ($activitygrade->itemname == 'Autoevaluación 360' || $activitygrade->itemname == 'Coevaluación 360' || $activitygrade->itemname == 'Heteroevaluación 360' || $activitygrade->itemname == 'Efectividad 360'|| $activitygrade->itemname == 'Entregables 360' )) {
                //LFAS 15-06-2024 Salta las actividades en caso de requerir hacer algo con ellas colocarlo en este apartado
                }else {
