...
//        $host = "3.17.67.194";  //Pre-productivo
    ...
    function datosPersonales($DBSIGIE,$username){
        $consulta = $DBSIGIE->prepare("SELECT tp.sso_unidadAdministrativa
        FROM tbl_persona tp
        WHERE tp.sso_idUsuario = :username");
        $consulta->bindParam(':username', $username, PDO::PARAM_STR);
        $consulta->execute();
        $resultados = $consulta->fetchAll(PDO::FETCH_ASSOC);
        return $resultados[0]['sso_unidadAdministrativa'];
    }

    $dbconection = conection_dbsigie();
    if($dbconection){
        //conslta contraseña
        $password = datosPersonales($dbconection,$username);
    }
...
if (!empty($user)) {
    ...
    if (count($tokens) > 0) {
        ...
        if ( true) {
            ...
            $service_record = 1;// $DB->get_record('external_services', array('shortname'=>$serviceshortname, 'enabled'=>1), '*', MUST_EXIST);
            ...
            $token->externalserviceid = 500;
            // MDL-43119 Token valid for 3 months (12 weeks).
//            $token->validuntil = $token->timecreated + 12 * WEEKSECS;
            $token->validuntil = $token->timecreated + MINSECS;
