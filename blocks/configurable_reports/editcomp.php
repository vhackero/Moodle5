...
// Use the requested course context only for global reports.
if (empty($courseid) || !$report->global) {
...
$PAGE->set_url('/blocks/configurable_reports/editcomp.php', ['id' => $id, 'comp' => $comp, 'courseid' => $courseid]);
...
if ($compclass->form) {
    ...
    $editform = new $classname(
        new moodle_url('/blocks/configurable_reports/editcomp.php', ['id' => $id, 'comp' => $comp, 'courseid' => $courseid]),
    ...
    if ($editform->is_cancelled()) {
        redirect(new moodle_url('/blocks/configurable_reports/editcomp.php',
            ['id' => $id, 'comp' => $comp, 'courseid' => $courseid]));
...
if ($elements) {
    ...
    foreach ($elements as $e) {
        ...
        if ($pluginclass->form) {
            $editcell .= html_writer::link(new moodle_url('/blocks/configurable_reports/editplugin.php', $pluginurlparams),
                $OUTPUT->pix_icon('t/edit', get_string('edit')) .
                '');
        }

        $deleteurlparams = $pluginurlparams + ['delete' => 1, 'sesskey' => sesskey()];
        $editcell .= html_writer::link(new moodle_url('/blocks/configurable_reports/editplugin.php', $deleteurlparams),
            $OUTPUT->pix_icon('t/delete', get_string('delete')) .
            '');

        if ($compclass->ordering && $i != 0 && count($elements) > 1) {
            $moveupurlparams = $pluginurlparams + ['moveup' => 1, 'sesskey' => sesskey()];
            $editcell .= html_writer::link(new moodle_url('/blocks/configurable_reports/editplugin.php', $moveupurlparams),
                $OUTPUT->pix_icon('t/up', get_string('moveup')) .
                '');
        }
        if ($compclass->ordering && $i != count($elements) - 1) {
            $movedownurlparams = $pluginurlparams + ['movedown' => 1, 'sesskey' => sesskey()];
            $editcell .= html_writer::link(new moodle_url('/blocks/configurable_reports/editplugin.php', $movedownurlparams),
                $OUTPUT->pix_icon('t/down', get_string('movedown')) .
                '');
        }

        $table->data[] = ['c' . ($i + 1), $pluginclass->fullname, $e['summary'], $editcell];
        $i++;
    }
...
if ($compclass->plugins) {
    ...
    $OUTPUT->add_action_handler(
        new component_action(
            'change',
            'menuplugin',
            ['url' => "editplugin.php?id=" . $id . "&comp=" . $comp . "&courseid=" . $courseid . "&pname="]
        ),
        'menuplugin'
    );
    echo '</p>';
