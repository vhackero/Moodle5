...
function export_report($report) {
    ...
    $graphfile = block_configurable_reports_export_download_graph($graphurl);
    if (!empty($graphurl) && empty($graphfile)) {
        throw new moodle_exception('error', 'block_configurable_reports', '',
            null, 'Could not render the report graph image for XLS export.');
    }
    ...
    if (!empty($graphfile)) {
        $myxls->insert_bitmap(0, 7, $graphfile, 0, 0, 0.5, 0.5);
    }

    $workbook->close();
    if (!empty($graphfile) && file_exists($graphfile)) {
        unlink($graphfile);
    }
    exit;
}
