...
abstract class report_base {
    ...
    /**
     * Check if the report has configured filters.
     *
     * @return bool
     */
    ...
    /**
     * Check if the current request includes at least one active filter value.
     *
     * @return bool
     */
    public function has_active_filter_request(): bool {
        $request = array_merge($_POST, $_GET);
        ...
        foreach ($request as $key => $val) {
            ...
            if (is_array($val)) {
                foreach ($val as $item) {
                    if ($this->is_active_filter_value($item)) {
                        return true;
                    }
                }
                continue;
            ...
            if ($this->is_active_filter_value($val)) {
                return true;
            }
        }
    ...
    /**
     * Check if a filter value should trigger report generation.
     *
     * @param mixed $value
     * @return bool
     */
    private function is_active_filter_value($value): bool {
        if (is_array($value)) {
    ...
    public function create_report(): bool {
        ...
        $this->finalreport->config = $this->config;
        $this->finalreport->graphs = $this->get_graphs($table->data);
    ...
    public function print_report_page(moodle_page $moodlepage) {
        ...
        if (empty($this->finalreport)) {
            ...
            echo html_writer::tag(
                'div',
                get_string('filter_required_to_run_report', 'block_configurable_reports'),
                ['class' => 'centerpara alert alert-info']
            );
