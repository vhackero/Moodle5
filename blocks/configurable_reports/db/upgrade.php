...
function xmldb_block_configurable_reports_upgrade($oldversion) {
    ...
    if ($oldversion < 2027050408) {
        $table = new xmldb_table('block_configurable_reports');
        $field = new xmldb_field('institution', XMLDB_TYPE_CHAR, '255', null, null, null, null, 'summaryformat');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_plugin_savepoint(true, 2027050408, 'block', 'configurable_reports');
    }
