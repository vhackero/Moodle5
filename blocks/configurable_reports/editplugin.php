...
if (!$pname) {
    redirect(new moodle_url('/blocks/configurable_reports/editcomp.php', ['id' => $id, 'comp' => $comp, 'courseid' => $courseid]));
    exit;
}
...
$displayreport = clone($report);
$displayreport->courseid = $courseid;
...
$reportclass = new $reportclassname($displayreport);
...
$PAGE->set_url('/blocks/configurable_reports/editplugin.php',
    ['id' => $id, 'comp' => $comp, 'cid' => $cid, 'pname' => $pname, 'courseid' => $courseid]);
...
$pluginclass = new $pluginclassname($displayreport);
...
if (isset($pluginclass->form) && $pluginclass->form) {
    ...
    if ($editform->is_cancelled()) {
        if (!empty($report)) {
            redirect(new moodle_url('/blocks/configurable_reports/editreport.php',
                ['id' => $report->id, 'courseid' => $courseid]));
        } else {
...
$PAGE->navbar->add($title, new moodle_url('/blocks/configurable_reports/editcomp.php',
    ['id' => $id, 'comp' => $comp, 'courseid' => $courseid]));
